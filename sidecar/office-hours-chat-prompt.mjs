  ...
  const lockedDay1GoalRules = isLockedDay1GoalFlow
    ? [
        ...
        ...(isGetUsersGoalFlow ? [
          ...
          "2) `get_users_first_candidate` / signalLabel `첫 후보 확정`: pin the ONE exact reachable person/handle/thread/source for this week's activation attempt — never a generic sourcing category. Availability is mechanical: a `## Verified Candidate Hints` section is present in the injected context AND non-empty. WHEN NO `## Verified Candidate Hints` section is injected (today's reality), the card MUST (a) NOT offer generic sourcing-category options (지인, 소개, 커뮤니티, 모임, 채널, 세그먼트, 비슷한 사람, 연락 가능한 사람, 어디서 찾을지 등 'where to find someone' buckets are forbidden as non-blocker options); (b) require the founder to NAME an exact reachable person/handle/thread/source in free text — phrase the question around `오늘 실제로 연락하거나 글을 올릴 수 있는 첫 사람의 실명·핸들, 또는 구체적 스레드·모임·채널` and put that instruction in the free-text placeholder and the primary option label; (c) include EXACTLY ONE explicit `아직 후보 없음` blocker that routes to an acquisition branch (오늘 후보 1명의 실명·핸들을 찾는 행동을 정함), not to an impossible 'must already have a reachable candidate' state; (d) NEVER fabricate a name, handle, or persona. IF a `## Verified Candidate Hints` section IS present and non-empty, instead build 1-3 options whose value is an exact hint id from that section (grounded options) plus the single blocker, and still forbid generic categories. Do NOT add a `직접 입력`, `기타`, or `other` option to capture the name — that is a contract violation; the founder names the candidate through the card's free-text answer (allowFreeText is already true), so phrase one non-blocker option as the named-capture instruction (e.g. `오늘 연락할 실명·핸들 적기`) without the literal `직접 입력` label.",
          ...
          "The get_users first-candidate card MUST NOT ask `고객 후보 1명은 어디에서 찾을 건가요?` or `어디서 잡을까요?` as a standalone sourcing question, and MUST NOT offer generic sourcing-category options (지인, 소개, 커뮤니티, 모임, 채널, 세그먼트, 비슷한 사람, 연락 가능한 사람) — those are 'where to find someone' buckets, not a confirmed candidate. It must pin ONE exact reachable person/handle/thread/source (named free text when no hints are injected) tied to this week's activation request and a concrete next action, with exactly one `아직 후보 없음` blocker. The blocker routes to an acquisition branch (오늘 후보 1명의 실명·핸들을 찾는 행동), never to an impossible 'must already have a reachable candidate' state. When the project context names a distinctive customer, preserve that customer's distinguishing conditions and exclusion signals — do not flatten them into a generic segment like `비슷한 사람`.",
