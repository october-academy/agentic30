...
Then SHOW it to the user via agentic30_request_user_input:
...
3. **agentic30_request_user_input per friction point.** One question per friction point found.
...
7. agentic30_request_user_input if there's a genuine DX choice to resolve
...
**STOP.** agentic30_request_user_input once per issue. Recommend + WHY. Reference the persona.
...
**STOP.** agentic30_request_user_input once per issue. Recommend + WHY.
...
**STOP.** agentic30_request_user_input for any item that requires a design decision.
...
Follow the agentic30_request_user_input format from the Preamble above. Additional rules for
DX reviews:

* **One issue = one agentic30_request_user_input call.** Never combine multiple issues.
...
* **Escape hatch (tightened):** If a section has zero findings, state "No issues,
  moving on" and proceed. If it has findings, use agentic30_request_user_input for each — a
  gap with an "obvious fix" is still a gap and still needs user approval before
  any change lands in the plan. Only skip agentic30_request_user_input when the fix is
...
agentic30_request_user_input. Never batch. For DX debt: missing error messages, unspecified upgrade
...
If any agentic30_request_user_input goes unanswered, note here. Never silently default.
...
Use agentic30_request_user_input with applicable options:
