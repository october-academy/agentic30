...
Then check for prior finalized.html. If `FINALIZED` was also found, use agentic30_request_user_input:
...
3. If in plan-driven or freeform mode (no approved PNG), design from context:
   ...
   - **Freeform:** use agentic30_request_user_input to gather what the user wants to build. Ask about:
...
If a framework is detected, use agentic30_request_user_input:
...
LOOP:
  ...
  3. agentic30_request_user_input (adjust wording based on mode):
...
Maximum 10 iterations. If the user hasn't said "done" after 10, use agentic30_request_user_input:
