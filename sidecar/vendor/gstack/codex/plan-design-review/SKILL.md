...
### 0D. Focus Areas
agentic30_request_user_input: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
**PRIMARY WAIT: agentic30_request_user_input with board URL**

After the board is serving, use agentic30_request_user_input to wait for the user. Include the
...
**Do NOT use agentic30_request_user_input to ask which variant the user prefers.** The comparison
board IS the chooser. agentic30_request_user_input is just the blocking wait mechanism.

**After the user responds to agentic30_request_user_input:**
...
6. The board auto-refreshes. **agentic30_request_user_input again** with the same board URL to
   wait for the next round of feedback. Repeat until `feedback.json` appears.

**If `NO_FEEDBACK_FILE`:** The user typed their preferences directly in the
agentic30_request_user_input response instead of using the board. Use their text response
as the feedback.

**POLLING FALLBACK:** Only use polling if `$D serve` fails (no port available).
In that case, show each variant inline using the Read tool (so the user can see them),
then use agentic30_request_user_input:
...
Use agentic30_request_user_input to verify before proceeding.
...
**Do NOT use agentic30_request_user_input to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use agentic30_request_user_input to confirm you understood the feedback correctly, never to re-ask what they chose.
...
5. agentic30_request_user_input if there's a genuine design choice to resolve
...
**STOP.** agentic30_request_user_input once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** agentic30_request_user_input once per issue. Do NOT batch. Recommend + WHY.
...
Each decision = one agentic30_request_user_input with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.
...
agentic30_request_user_input: "The review passes changed [list major design changes]. Want me to regenerate mockups to reflect the updated plan? This ensures the visual reference matches what we're actually building."
...
Follow the agentic30_request_user_input format from the Preamble above. Additional rules for plan design reviews:
...
* **Escape hatch (tightened):** If a section has zero findings, state "No issues, moving on" and proceed. If it has findings, use agentic30_request_user_input for each — a gap with an "obvious fix" is still a gap and still needs user approval before any change lands in the plan. Only skip agentic30_request_user_input when the fix is genuinely trivial AND there are no meaningful design alternatives. When in doubt, ask.
* **NEVER use agentic30_request_user_input to ask which variant the user prefers.** Always create a comparison board first (`$D compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use agentic30_request_user_input ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual agentic30_request_user_input. Never batch TODOs — one per question. Never silently skip this step.
...
If any agentic30_request_user_input goes unanswered, note it here. Never silently default to an option.
