...
If the output is non-empty (working tree is dirty), **STOP** and use agentic30_request_user_input:
...
**If NO runtime detected** (no config files found): Use agentic30_request_user_input:
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. agentic30_request_user_input: "This site requires authentication. Want to import cookies from your browser? Run `/setup-browser-cookies` first if needed."
...
11. **Clean working tree required.** If dirty, use agentic30_request_user_input to offer commit/stash/abort before proceeding.
