...
### A′ evidence-receipt cutover (Office Hours graded attempt evidence)

The four GRADED ValidationAttempt transitions (`record_action_proof` / `record_customer_outcome` / `record_negative_outcome` / `record_goal_proof`) are gated by a host-SIGNED evidence receipt, not a self-attested `{kind,ref}`. A raw `{kind,ref}` payload now fails with `receipt_required` (NO backward compat). Pipeline: `office_hours_ingest_evidence` (bytes → `office-hours-artifact-blob-store.mjs` content-addressed durable write → `office-hours-artifact-registry.mjs` register → `office-hours-evidence-ingress.mjs#ingestSwiftUpload` signs a `swift_upload`/`message.sent` receipt) returns a token; the founder submits it via `office_hours_attempt_evidence` with `evidence.receipt`. `handleOfficeHoursAttemptEvidence` (index.mjs) then: `prepareReceiptEvidence` (`office-hours-evidence-coordinator.mjs` — verify against durable binding authority + ANY-OF claim → host-minted reducer kind via `office-hours-evidence-policy.mjs`, the single transition→{requiredGrade,claimsAnyOf,canonicalKind} source) → idempotent-retry guard → EXACT-transition dry-run (`projectAttemptFromEvents` on `[...events, candidate]`, reject illegal from-state BEFORE consume so a valid receipt is never single-use-burned) → `consumeArtifact` (single-use) → `commitAttemptEvent`. Binding ids come from durable authority: `installActorId`+per-workspace `projectId`+per-project HMAC keyring live in `office-hours-host-identity.mjs` OUTSIDE the workspace (`AGENTIC30_HOST_STORE_DIR`, default `~/Library/Application Support/agentic30`, 0700/0600 — the MAC key must not be reachable by the model's workspace-scoped tool surface; the secret is never passed via env/argv/WS/logs); `evidenceContractId` is derived (`office-hours-evidence-binding.mjs#deriveEvidenceContractId`) from the attempt projection's contract fields so a superseded contract re-derives a new id and invalidates old receipts. Provenance (verifiedClaim/origin/trustTier/artifactId/sha256/receiptDigest) lives in `fields.evidence` (the store audit normalizer drops unknown audit keys; `gradeEvidence` spreads the evidence record so it survives). The host-minted goal kind is `activation_event` and refs stay `sourceType:"office_hours_attempt"`, so the strong-payment gate (proof_ledger + strength:"strong") is unreachable from an attempt by construction. MINIMAL-SCOPE (Oct 2026-06-26): only the `swift_upload` ingress adapter ships → action proof is reachable; `customer_outcome`/`goal_proof` need a `recipient_callback`/`provider_event` adapter (deferred until a real provider hookup) and honestly stop at `awaiting_customer_outcome`. The eval harness (`sidecar-evals/real-project-arc.mjs`) signs synthetic recipient/provider receipts (eval-epoch, shared host store) to exercise the full chain.
