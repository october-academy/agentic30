...
| `agent-work-history.mjs` | Deterministic, redacted digest of recent agent work on a workspace from `~/.claude` (encoded project dir) + `~/.codex` (grep + cwd verify). cli-first (excludes Agentic30's own sdk-ts runs), streaming, KST-bucketed. Pure record→event reducers exported for tests |
...
| `generate-day1-situation-summary.mjs` | Day-1 multi-angle "project situation" summary (product/engineering/recent-focus angles + README update + next actions + button-first goal decision). Deterministic floor + optional Claude refinement; broadcast on `workspace_scan_result.day1SituationSummary` |
