...
function insertFrameRow(store, overrides = {}) {
  const row = {
    ...
    snapshot_asset_id: "asset-live-1",
...
test("isSeedFixtureFrameRow flags the UI seed markers, not live rows", () => {
  ...
  assert.equal(
    isSeedFixtureFrameRow({ id: "frame-abc", capture_trigger: "manual_swift_screencapturekit", snapshot_asset_id: "asset-abc" }),
    false,
  );
});

test("isLiveCapturedFrameRow requires a real collector trigger, non-seed asset, and non-deleted state", () => {
  const live = { id: "frame-abc", capture_trigger: "auto_swift_screencapturekit_interval", snapshot_asset_id: "asset-abc" };
  assert.equal(isLiveCapturedFrameRow(live), true);
  assert.equal(
    isLiveCapturedFrameRow({
      id: "frame-event",
      capture_trigger: "auto_swift_event_tap_mouse_down",
      snapshot_asset_id: "asset-event",
    }),
    true,
  );
  assert.equal(
  ...
  assert.equal(isLiveCapturedFrameRow({ id: "frame-abc", capture_trigger: "app_switch", snapshot_asset_id: "asset-abc" }), false);
  // Deleted → not live.
  assert.equal(isLiveCapturedFrameRow({ ...live, deleted_at: "2026-06-27T10:00:00.000Z" }), false);
  // Missing/seed asset → not live.
  assert.equal(isLiveCapturedFrameRow({ id: "frame-abc", capture_trigger: "auto_swift_screencapturekit_interval", snapshot_asset_id: "" }), false);
});
...
test("assertLiveRecorderFrameRow accepts a live row and fails closed on seed/missing", async () => {
  ...
  try {
    ...
    insertFrameRow(store, {
      id: "frame-live-event",
      capture_trigger: "auto_swift_event_tap_mouse_down",
      snapshot_asset_id: "asset-live-event",
    ...
    const liveRow = assertLiveRecorderFrameRow(store, "frame-live-1");
    assert.equal(liveRow.id, "frame-live-1");
    const eventTapRow = assertLiveRecorderFrameRow(store, "frame-live-event");
...
test("summarizeLiveRecorderCapture separates live from seeded frames", async () => {
  ...
  try {
    ...
    assert.deepEqual(summary.liveFrameIds, ["frame-live-1", "frame-live-event"]);
