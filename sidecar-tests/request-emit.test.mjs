...
test("Office Hours Day 1 get_users uses generic LLM card path after first answer", async () => {
  const harness = await spawnSidecar({
    extraEnv: {
      AGENTIC30_TEST_STUB_OFFICE_HOURS_MCP_REQUEST: "1",
    },
  ...
  try {
    ...
    ws.send(JSON.stringify({
      ...
      visiblePrompt: "Test Day 1 get_users generic path",
      source: "day1_interview_goal_locked",
      day: 1,
    }));

    const firstPrompt = await waitForPendingOfficeHoursPrompt(ws, created.session.id);
    assert.equal(firstPrompt.generation?.mode, "office_hours_tool");
    assert.equal(firstPrompt.generation?.attemptId, undefined);
    assert.equal(firstPrompt.generation?.expectedRevision, undefined);

    const marker = ws.events.length;
    submitStructuredAnswer(ws, created.session.id, firstPrompt, {
      selectedOptions: [],
      freeText: "프로젝트를 연결하고 오늘의 고객 검증 행동을 정리한 1명",
    });

    const nextPrompt = await waitForEvent(ws.events, (event) =>
      ws.events.indexOf(event) >= marker
        && event.type === "session_updated"
        && event.session?.id === created.session.id
        && event.session?.status === "awaiting_input"
        && event.session?.pendingUserInput?.requestId
        && event.session.pendingUserInput.requestId !== firstPrompt.requestId
    );
    assert.equal(nextPrompt.session.pendingUserInput.generation?.attemptId, undefined);
    assert.equal(
      ws.events.slice(marker).some((event) =>
        event.type === "office_hours_status"
          && event.sessionId === created.session.id
      ...
      true,
      "get_users continuation should run through the generic provider path",
    );
    ...
    assert.equal(
      turnLog.turns.some((turn) =>
        turn.day === 1
          ...
          && turn.requestId === firstPrompt.requestId
          && /프로젝트를 연결/.test(turn.responseText)
      ),
      true,
      "generic get_users answer must be written to the Office Hours turn log",
    );
  } finally {
    ws?.close();
    await harness.close();
