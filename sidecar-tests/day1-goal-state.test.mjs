...
test("Day 1 goal normalization strips duplicated problem text from customer and goal", () => {
  ...
  assert.equal(
    selection.goalText,
    "전업 1인 개발자 (수익 0원, macOS)가 \"만들 줄은 알지만 무엇을 팔아야 하는지 모른다\" 문제를 실제 유입/가입 행동으로 반복해서 드러내는지 확인한다. 방법: 지불 의향과 현재 대안을 첫 고객 대화에서 묻는다.",
  );
});
...
test("Day 1 goal normalization repairs legacy sentence-composition artifacts", () => {
  ...
  assert.equal(
    selection.goalText,
    "전업 1인 개발자 (수익 0원, macOS)가 \"만들 줄은 알지만 무엇을 팔아야 하는지, 어떻게 사람을 데려와야 하는지, 오늘 무엇을 검증해야 하는지 모른다\" 문제에 돈이나 시간을 쓸지 확인한다. 방법: 지불 의향과 현재 대안을 첫 고객 대화에서 묻는다.",
  );
