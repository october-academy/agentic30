...
export function buildJudgePrompt({ scenario, referenceDocs, observed }) {
  return [
    ...
    "",
    "Rubric, each 0-10:",
    "- icp_fit: observed behavior adapts to the USER'S OWN target customer, not to any built-in persona. Score higher when the observed cards/output (1) preserve the project's distinctive customer conditions and exclusion signals from the injected context instead of flattening them into a generic segment like 비슷한 사람; (2) tie the first candidate to a reachable person/source AND make a concrete validation action requestable today; (3) when the injected project context is thin or generic, AVOID inventing a customer persona, NEVER assume the customer is a developer or a macOS user, and NEVER inject Agentic30's own ICP (full-time solo macOS/Codex developer, zero revenue) onto the user's project; (4) define the active-user bar by the project customer's observable validation behavior (a completed core action), not by demographics.",
    ...
    "- Do not lower icp_fit because the observed output fails to confirm the user is a full-time solo macOS/Codex developer with zero revenue and interview willingness; those Agentic30 ICP facts are criteria for whether AGENTIC30 should serve this user, not behavior the Day 1 get_users surface is required to produce.",
    "- For get_users surfaces, treat preserving the project's own distinctive customer conditions and a reachable + requestable validation action as the primary positive icp_fit evidence; treat injecting a developer/macOS persona onto a project whose injected context does not name one as an icp_fit regression, not a strength.",
    "- When the injected project context is thin (placeholder text such as 좁히는 중 / 확인 필요 / a default 100-user goal), reward observed behavior that honestly asks the user to name the product/customer/problem and avoids fabricating specifics; do not reward confident-but-invented customer detail.",
    "",
