...
- Set `POSTHOG_PROJECT_API_KEY` (and optionally `POSTHOG_HOST`) before archiving so Info.plist embeds the project token (`Agentic30PostHogProjectAPIKey` / `Agentic30PostHogHost`). `scripts/build-and-notarize.sh` blocks release builds when the token is missing or the exported app does not contain a `phc_…` token. The Settings opt-out toggle (`agentic30.posthog.telemetryDisabled` UserDefault) lets end users disable per-device.
