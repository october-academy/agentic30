...
1. **30일 커리큘럼 콘텐츠는 이미 코드에 저작되어 있다.** `IDD_BASE_CURRICULUM`이 Day 1–30 전체(foundation 1–7 / build 8–17 / launch 18–24 / grow 25–30)를 정의한다 (`sidecar/adaptive-curriculum.mjs:182-213`). 빠진 것은 콘텐츠가 아니라 **배선**이다: Day 2+ 미션이 day-progress 매크로 루프·증거 시스템·Office Hours와 연결되지 않았다.
...
3. **signal은 이미 풍부하게 수집되지만 커리큘럼을 바꾸지 못한다.** rushing 감지(`prior-day-execution-signals.mjs:60-67`), build-without-customer-evidence(`daily-office-hours-digest.mjs:913`), abandoned-thread 탐지(`office-hours-memory.mjs:477-489`), pace 분류(`review-day-summary.mjs:323-387`)가 모두 계산되지만 표시용이다. `personalizeDay()`는 signal을 받아도 미션 내용을 바꾸지 않는다 (`adaptive-curriculum.mjs:220-221`).
...
- `curriculum-progression-gate.mjs:41-114`: Day 2–7은 직전 Day의 proof-ledger 이벤트(actionEvidence/interview/dmAsk)가 `accepted|verified`여야 통과. Day 8+은 추가로 Day 7의 `dayDecision`(continue/pivot/stop) 필요.
...
- proof-ledger: 11개 이벤트 타입(setup/mission/interview/bip/workLog/dmAsk/landingMetric/paymentIntent/actionEvidence/dayDecision/referral), status 8종, **strength weak/medium/strong 필드가 이미 존재** (`execution-os.mjs:17-29`, `:138-184`). 저장: `.agentic30/proof-ledger.json`.
...
- 시작: 사용자/Mac-auto 수동 (`sidecar/index.mjs:1292-1310`, `/office-hours` slash `:3396-3399`). **시스템 선제 트리거 없음.**
...
| adaptiveDifficultyState | `review-day-curriculum-signals.mjs:827-851` | **normalize만 하고 쓰는 곳 없음** |
...
3. **Day 2+ (부분 구현):** morning briefing(탭 진입 시 날짜 stale이면 수집, `morning-briefing.mjs:128-145`) → office hours(수동 시작, source gate, 일일 digest 주입) → 커밋먼트 제안/확정 → 증거 제출(office_hours_commitment_evidence). execution 스텝의 미션 콘텐츠는 IDD 커리큘럼과 미연결.
...
| 알림 설정 | `<ws>/.agentic30/curriculum-notification-settings.json` | v1 | 21:00 고정 리마인더 |
...
2. weak-only 진행이 2 Day 연속이면 risk signal로 OH intervention 후보 (§12 AR-17).
3. 커밋먼트 hard-evidence 4종(url/screenshot/commit/payment, `office-hours-memory.mjs:43`)은 유지하되, commit은 "고객 대상 커밋먼트"의 증거로는 불인정하도록 expectedEvidenceKind 검증을 추가 (§15.5).
...
- 기존 `evaluateCurriculumProgressionGate`(`curriculum-progression-gate.mjs:41-114`)는 Day 2–7 proof gate로 그대로 사용하고, Gate Engine이 이를 포함(위임)한다 — 비파괴 확장.

### 10.2 Milestone gates (하드블록 + OH 회부, Decision D3·D5)

| Gate | 시점 | 통과 조건 (모두 strong 증거) | 실패 시 |
|---|---|---|---|
| G1 첫 고객 접촉 | Day 1 | 기존 first_interview gate 유지: 커밋먼트 or confession (`index.mjs:3240-3266`) — **추가:** Day 3까지 인터뷰 evidence(proof-ledger interview, strong) 미제출 시 G1을 retro-block | confession 경로 + AR-02 |
...
| G6 revenue 검증 상태 | Day 28 review | paymentRecord ≥1 **또는** (paymentIntent ≥3 + 명시적 거절 기록) — "지불 행동 또는 명시적 가격 거절의 증명" (`adaptive-curriculum.mjs:209` Day 27 원문) | Day 29 진입은 허용하되 Day 30 결정 옵션에서 continue 제한(§12 AR-15) |
...
- evidence: day1-goal.json 저장(시스템 기록, strong) + first_interview 커밋먼트(고객·채널·메시지·기대증거·기한).
...
- OH trigger: confession 시 즉시 mini-intervention(왜 못 보냈는지 1문항).
...
- gate: Day 2–7 proof gate(accepted/verified 필요, `curriculum-progression-gate.mjs:41-114`). adaptive: 증거 0이면 mini-action으로 시장 1개 좁히기 재시도.
...
- mission: monetization-ask 4턴 플로우(target→draft→sent→response, `monetization-ask-state.mjs:40-69`).
...
- evidence: foundation-summary 산출(go-no-go.md) + dayDecision(continue/pivot/stop/restart, strong=시스템 기록).
...
- gate: Day 8 진입 자체가 G2 통과를 전제 (`curriculum-progression-gate.mjs` Day 8+ 규칙).
...
- evidence: activation baseline(HogQL 자동, strong). adaptive: active user 증가율로 Day 26–27 순서 조정(AR-13).
...
| AR-03 | 인터뷰 했지만 learning 약함 | 인터뷰 N≥3인데 답변에 과거행동·대안·지불 신호 0 (judge 기준) | medium | "칭찬만 모았다. Mom Test 위반" | 질문 스크립트 재작성 mini-action | 허용 | 2회 반복 시 회부 |
| AR-04 | 너무 쉬운 mission 반복 | education/정리형만 완료, action·interview 회피 3일 | medium | "안전한 일만 골랐다" | 다음 Day를 interview로 강제 | 허용 | 회피 5일 시 |
...
| AR-16 | 증거 조작/부실 의심 | judge insufficient 반복 ≥3 + 동일 증거 재제출, 시각/발신자 식별 불가 | medium | "이 증거로는 판정 불가" | 원본 증거 요구(원문·시각 포함) | milestone 차단 | 2회 시 회부 |
| AR-17 | 허위 진행(커밋먼트 코스튬) | abandoned threads ≥1 (`office-hours-memory.mjs:477-489`) + 신규 커밋먼트 계속 생성 | high | "N사이클째 증거 0인 약속 위에 새 약속을 쌓고 있다" | 기존 커밋먼트 1개 닫기 전 신규 금지 | 신규 커밋먼트 차단 | 즉시 회부 |
| AR-18 | rushing | rushingCandidate=true (`prior-day-execution-signals.mjs:60-67`) 또는 pace=rusher | high | "속도가 아니라 증거 밀도가 목표다" | 다음 day 분량 축소+검증 강화 | 허용 | — |
| AR-19 | 반복 정지(이탈 전조) | day-progress 3일 무갱신 + 앱 실행 있음 | medium | "3일째 멈췄다. 가장 작은 다음 행동 1개" | 21:00 리마인더 강화+1-step mission | 허용 | 5일 시 회부 |
...
2. trigger별 고정 질문 세트(예: AR-01 → "오늘 커밋이 검증한 고객 가설은 무엇인가? 없다면 무엇이 두려운가"; AR-14 → "결제 직전 무엇이 끊겼는지 고객 원문으로 말하라").
3. 종료 조건: **구조화 커밋먼트 1개(고객·채널·메시지·기대증거·기한, user-origin) 확정** — 기존 commitment-suggest→confirm 플로우 그대로 (`office-hours-commitment-suggest.mjs`).
4. 필수 post-session evidence: 커밋먼트의 expectedEvidenceKind에 따른 strong 증거, dueDay 내.

### 13.4 효과와 불참 처리

- 참여+커밋먼트 확정: 차단형이었으면 1회 한정 통과 토큰(confession 경로와 동일 의미) 발급, gate-ledger에 기록.
...
`locked → open(조건 충족 가능) → passed | blocked(→ OH intervention → passed-via-confession | 유지)`. milestone gate에 waive 없음(Decision D3). 평가는 멱등·재실행 가능(증거 도착 시 재평가).
...
- `evidence.kind == expectedEvidenceKind` 검증 추가, 고객 대상 커밋먼트에 `commit` kind 불인정. carried_forward 체인에 `satisfiedByLaterVersion` 링크(텍스트 드리프트 중복 해소, 현재 dedup 한계 — `office-hours-memory.mjs:818-832`).

### 15.6 adaptiveDifficultyState 기록 시작 (curriculum progress 내, 스키마 무변경)

review day 종료 시 `{ direction: lighter|hold|heavier, trigger, appliedWeek }` 기록 — normalize 코드는 이미 존재(`review-day-curriculum-signals.mjs:827-851`), writer만 신설.
...
| commitmentGate UI 완성 | published 상태는 있으나 미표면(§ swift 관찰 — commitmentGateMessage/Step 존재) | `AgenticViewModel.swift` |
...
기존: 21:00 미완료 리마인더 1종, 설정 schema v1 (`curriculum-notification-scheduler.mjs:8-80`, `curriculum-notification-settings.mjs`). 확장(모두 같은 스케줄러에 추가, 일일 상한 2건·기존 lastSent 중복 방지 로직 재사용):

1. **gate-blocked 아침 알림** (브리핑 시각): "G4가 잠겨 있다 — 필요한 증거 1개".
2. **커밋먼트 dueDay 알림**: 기한 당일 09:00.
3. **AR-19 정지 알림**: 3일 무갱신 시 21:00 리마인더를 1-step mission 문구로 교체.
4. graduated/terminal 시 전체 중지(기존 로직 유지, `adaptive-curriculum.mjs:781-786`).

---

## 20. Admin/Mentor Dashboard (Decision D6)
...
| Failure | 영향 | 대응 |
|---|---|---|
| PostHog/Cloudflare 소스 다운·미연동 | AR-08~15·G4②·G5 판정 불가 | 자동집계 rule 발동 금지(§12 오탐 대응 ③), gate는 "판정 보류" 상태로 두고 캡처 증거 임시 경로(medium, 1회 한정) 허용 + 소스 복구 시 재평가. 부분 digest 허용(현재 전면 차단인 source gate를 partial로 완화 — `daily-office-hours-digest.mjs:310-431` 변경안) |
...
8. **telemetry 컨텍스트 보강 + 신규 이벤트 + PostHog 코호트 대시보드** — §16·§20
...
- 전용 클라우드 mentor 대시보드, prestige tier 운영 도구
