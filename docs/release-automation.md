...
5. **GitHub release shows only one DMG or a failed run left release assets attached** — one arch matrix job failed or the final publish step did not complete. With the draft gate, the release never becomes public unless both DMGs are attached; the cleanup job deletes any leftover draft release object on failure and leaves the tag intact. Fix the root cause, bump the build, and cut a fresh tag. (Incident that motivated the draft gate: `v20260611-0738` published an Intel-only build 16 release from a half-failed run.)
