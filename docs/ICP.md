...
에이전트 코딩 도구로 제품을 만들 수 있지만, 아직 첫 매출을 만들지 못한 전업 1인 개발자. macOS를 쓰고, 30일 동안 프로젝트 기록을 남기며 실행할 의향이 있어야 한다.
...
- 첫 매출 전
- macOS 사용자
- Claude Code/Codex/Gemini/Cursor 등 에이전트 코딩 도구 사용
- 내가 개발중인 프로젝트 path 지정 가능
...
| **Haves** | Claude Code, Codex, Cursor, Gemini Pro (Max) Plan |
...
말보다 행동으로 검증한다. Agentic30의 좋은 ICP 신호는 다음과 같다.
...
| **온보딩** | Node/provider 셋업 → 프로젝트 path 선택 → 프로젝트 기록 연결을 먼저 완료한다. |
